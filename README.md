...
#### Invoice extracted by AI (2026)
- OCR → LLM pipeline: Extracts text from invoice images using PaddleOCR, then sends it to a local Phi-3 model (via Ollama) to parse structured fields (invoice number, date, vendor, total) into JSON and append to a CSV.
- Fully local / offline: Runs entirely on-device — no cloud APIs required; works even on Intel CPU, though GPU would be faster.
-	[GitHub](https://github.com/Jerrysuper123/extractDetailsFromInvoice)

#### Garmin watch face (2026)
